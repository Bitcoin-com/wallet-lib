...
class BCHWallet:
    ...
    def get_transactions(self, label:str=None, count:int=25, offset:int=0):
        label_str = label or '""'
        res = self.adapter.run(self._GET_TRANSACTIONS_COMMAND, label_str, str(count), str(offset))
        if res.error:
    ...
    def send(self, sender:str=None, recipient:str=None, amount:int=0):
        ...
        res = self.adapter.run(self._SEND_FROM_COMMAND, sender, recipient, str(amount))
        if res.error:
    ...
    def get_transactions_since(self, block_hash):
        res = self.adapter.run(self._GET_TRANSACTIONS_SINCE_COMMAND, block_hash)
        if res.error:
